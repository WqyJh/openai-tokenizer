...
- 🎯 Entry points to execute your program using `python -m <openai_tokenizer>` or `$ openai_tokenizer` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/WqyJh/openai-tokenizer/branch/main/graph/badge.svg?token=openai-tokenizer_token_here)](https://codecov.io/gh/WqyJh/openai-tokenizer)
[![CI](https://github.com/WqyJh/openai-tokenizer/actions/workflows/main.yml/badge.svg)](https://github.com/WqyJh/openai-tokenizer/actions/workflows/main.yml)

Awesome openai_tokenizer created by WqyJh

## Install

from source
```bash
git clone https://github.com/WqyJh/openai-tokenizer openai_tokenizer
cd openai_tokenizer
make install
```
...
```bash
pip install openai_tokenizer
```

## Executing

```bash
$ openai_tokenizer run --port 8080
```

or

```bash
python -m openai_tokenizer run --port 8080
```

or

```bash
$ uvicorn openai_tokenizer:app
```

## CLI

```bash
❯ openai_tokenizer --help
Usage: openai_tokenizer [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ openai_tokenizer create-user --help
Usage: openai_tokenizer create-user [OPTIONS] USERNAME PASSWORD
...
```bash
openai_tokenizer create-user admin admin --superuser
```
...
Run with `openai_tokenizer run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `openai_tokenizer create-user`.
...
-----------------------------------------------------
openai_tokenizer/__init__.py              4      0   100%
openai_tokenizer/app.py                  16      1    94%
openai_tokenizer/cli.py                  21      0   100%
openai_tokenizer/config.py                5      0   100%
openai_tokenizer/db.py                   10      0   100%
openai_tokenizer/models/__init__.py       0      0   100%
openai_tokenizer/models/content.py       47      1    98%
openai_tokenizer/routes/__init__.py      11      0   100%
openai_tokenizer/routes/content.py       52     25    52%
openai_tokenizer/routes/security.py      15      1    93%
openai_tokenizer/routes/user.py          52     26    50%
openai_tokenizer/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in openai_tokenizer/default.toml.
